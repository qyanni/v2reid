...
def resize_pos_embed_4d(posemb, posemb_new):
    ...
    posemb_grid = F.interpolate(posemb_grid, size=(gs_new, gs_new), mode='bicubic', align_corners=True)  # [1, dim, 14, 14] -> [1, dim, 24, 24]
...
def load_state_dict(checkpoint_path, model, use_ema=False, num_classes=1000):
    ...
    if checkpoint_path and os.path.isfile(checkpoint_path):
        ...
        if num_classes != 1000:
            ...
            #del state_dict['patch_embed.proj.weight']  ####### delete and add it .. normalized with correct shape
        ...
        if model.pos_embed.shape != old_posemb.shape:  # need resize the position embedding by interpolate
            ...
            if len(old_posemb.shape) == 3:
                ...
                #if cls_input before trasnformers in ViT instead of CaiT
                if len(model.pos_embed.shape) == 3:
                    new_posemb = torch.empty(model.pos_embed.shape)
                    nn.init.normal_(new_posemb, std=.02)
                else:
                    new_posemb = resize_pos_embed_4d(old_posemb, model.pos_embed)
         
            state_dict['pos_embed'] = new_posemb

        new_patch = model.patch_embed.proj.weight
        old_patch = state_dict['patch_embed.proj.weight']
        if new_patch.shape != old_patch.shape: 
            w = torch.empty(new_patch.shape)
            nn.init.normal_(w,std=0.09)
            state_dict['patch_embed.proj.weight'] = w
