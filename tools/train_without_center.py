...
def train_without_center(
    ...
    for epoch in range(1, cfg.SOLVER.MAX_EPOCHS+1):
        ...
        model.train() # here optimizer.step() is called numerous times.
    
        last_idx = len(loader) - 1 #len(loader) =  dataset size / batch size
        ...
        writer.add_scalars('mAP and CMC per epoch', {'mAP': mAP ,
                                                     'rank-1': cmc[0],
                                                     'rank-5': cmc[4],
                                                     'rank-10': cmc[9],
                                                     }, epoch) 
