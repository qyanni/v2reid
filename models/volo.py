...
#  Explanations on initialization:
...
class OutlookAttention(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.shape  # torch.Size([32, 28, 28, 192])
...
class ClassBlock(nn.Module):
    ...
    def forward(self, x):
        cls_embed = x[:, :1] #torch.Size([32, 1, 384])
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, stem_stride=1,
                 patch_size=16, in_chans=3, hidden_dim=64, embed_dim=384):
        super().__init__()
...
# add this, inspo from TransReid and PVTv2
# https://github.com/whai362/PVT/blob/v2/classification/pvt_v2.py
class PatchEmbed_overlap(nn.Module):
    """ Image to Patch Embedding with overlapping patches
    """
    def __init__(self, img_size=224, stride_size=4, patch_size=7, in_chans=3,
                hidden_dim=64,embed_dim=384):

        super().__init__()

        assert patch_size > stride_size, "Set larger patch_size than stride"

        self.H, self.W = img_size // stride_size, img_size // stride_size
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size,
                       stride=stride_size, padding=(patch_size // 2, patch_size // 2))
...
class VOLO(nn.Module):
    ...
    """
    def __init__(self, layers, img_size=224, in_chans=3, num_classes=1000, patch_size=8,# patch_size=8,
                 ...
                 post_layers=None, return_mean=False, 
                 pooling_scale=2, out_kernel=3, out_stride=2, out_padding=1,
                 overlap=False,  neck = 'off'):

        super().__init__()
        self.num_classes = num_classes
        if overlap:
            print(">>> Patch embedding with overlap")
            stride_size=5
            patch_size=8
            ...
            self.pos_embed = nn.Parameter(torch.zeros(1, img_size // stride_size // pooling_scale,
                            img_size // stride_size // pooling_scale , embed_dims[-1]))
            
        else:
            print(">>> Patch embedding without overlap using multiple convs")
            pooling_scale=2

            self.patch_embed = PatchEmbed( stem_stride=2, patch_size=patch_size,
        ...
        # Classifier head
            ...
            self.bottleneck = nn.BatchNorm1d(embed_dims[-1])
            self.bottleneck.bias.requires_grad_(False)
            self.head = nn.Linear(embed_dims[-1], num_classes, bias=False)
            self.bottleneck.apply(weights_init_kaiming)
            self.head.apply(weights_init_classifier)

        elif self.neck == "lnneck":
            self.bottleneck = nn.LayerNorm(embed_dims[-1])
    ...
    def get_classifier(self):
        # The name of the classification head of network can differ between networks.
        # To provide a consistent interface for different models, timm models have the get_classifier method, 
        # which can be used to retrieve the classification head without having to lookup the module name.
        return self.head

    def reset_classifier(self, num_classes):
        # can also modify the classifier and pooling layers of an existing model, using the reset_classifier method
        self.num_classes = num_classes
        self.head = nn.Linear(
    ...
    def forward_cls(self, x):
        B, N, C = x.shape #torch.Size([32, 196, 384])
        cls_tokens = self.cls_token.expand(B, -1, -1) #torch.Size([32, 1, 384])
        
        x = torch.cat((cls_tokens, x), dim=1) #torch.Size([32, 197, 384])
    ...
    def forward(self, x):
        ...
        if self.neck == 'off':           
            feat = global_feat
        elif self.neck == 'bnneck': # use batch norm
            feat = self.bottleneck(global_feat)
        elif self.neck == 'lnneck': # use layer norm
            feat = self.bottleneck(global_feat)
        x_cls = self.head(feat)
        # return these: 1. class token, 2. global feat for triplet loss
...
@register_model
def my_volo(pretrained=False, **kwargs):
    """
    My VOLO model:
    change the parameters in the brackets to whatever you wish
    """
    layers = [12, 12, 20, 4]
    embed_dims = [384, 768, 768, 768]
